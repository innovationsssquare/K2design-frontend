...
import { GetArtframecalculation } from "@/lib/ReduxSlice/Modernproduct/artframeSlice";
...
const Artframe = () => {
  ...
  const { Artframeresult, loading, error } = useSelector(
    (state) => state.Artframe
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetArtframecalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Artframeresult?.message === "Configuration not found") {
      ...
      <div className="bg-[#f1f2f4] flex justify-center items-center w-full">
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">Art Frame</h1>
            <p className="text-sm text-gray-600 font-bold mb-4">
              High-quality art frame in Terra finish with foam/canvas options.
            </p>

            <p class="mt-4 text-sm font-medium text-[#606060]">Available In:</p>

            <ul className="list-disc list-inside mb-4">
              <li>Steller - 50 mm Depth</li>
              <li>Lunar - 50 mm Depth</li>
              <li>Terra - 50 mm Depth</li>
            </ul>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    ...
                    <SelectItem value="Steller - 50 mm Depth">Steller - 50 mm Depth</SelectItem>
                    <SelectItem value="Lunar - 50 mm Depth">Lunar - 50 mm Depth</SelectItem>
                    <SelectItem value="Terra - 50 mm Depth">Terra - 50 mm Depth</SelectItem>
                  </SelectGroup>
                </SelectContent>
              </Select>
            </div>
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    ...
                    <SelectItem value="Silver Anodized">Silver Anodized</SelectItem>
                    <SelectItem value="Black Anodized">Black Anodized</SelectItem>
                    <SelectItem value="Bronze Anodized">Terra - 50 mm Depth</SelectItem>
                  </SelectGroup>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={Artframeresult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Artframeresult?.finalRate || 0}
                    </strong>
                  )}
                </Button>
