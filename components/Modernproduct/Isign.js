...
import { GetIsigncalculation } from "@/lib/ReduxSlice/Modernproduct/isignSlice";
...
const Isign = () => {
  ...
  const { Isignresult, loading, error } = useSelector((state) => state.Isign);
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetIsigncalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Isignresult?.message === "Configuration not found") {
      ...
      <div className="bg-[#f1f2f4] flex justify-center items-center w-full">
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">i-Sign</h1>
            <p className="text-sm text-gray-600 font-bold mb-4">
              Wall-mounted i-Sign with Ecosolvent print and foamsheet, available
              in Silver, Black, and Bronze anodized.
            </p>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button className="bg-white  " disabled={Isignresult == null}>
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Isignresult?.totalPrice || 0}
                    </strong>
                  )}
                </Button>
