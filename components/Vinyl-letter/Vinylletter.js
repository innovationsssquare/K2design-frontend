...
import { GetVinyllettercalculation } from "@/lib/ReduxSlice/Vinyl-letter/VinylletterSlice";
...
const Vinylletter = () => {
  ...
  const { Vinylletterresult, loading, error } = useSelector(
    (state) => state.Vinylletter
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetVinyllettercalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Vinylletterresult?.message === "Configuration not found") {
      ...
      <div className="bg-[#f1f2f4] flex justify-center items-center w-full">
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">Vinyl Letters & Numbers</h1>
            <p className="text-sm text-gray-600 font-bold mb-4">
             Custom cut vinyl letters & numbers in 1 Colour, Multi Colour, and Gold & Silver.
            </p>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="mb-4">
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="1 Colour">
                     One Colour
                    </SelectItem>
                    <SelectItem value="Multi Colour">
                     Multi Colour
                    </SelectItem>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={Vinylletterresult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Vinylletterresult?.totalPrice || 0}
                    </strong>
                  )}
                </Button>
