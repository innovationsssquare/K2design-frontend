...
import { GetPvcfoamplatescalculation } from "@/lib/ReduxSlice/Rigidsignplate/PvcfoamplatesSlice";
...
const Pvcfoamplates = () => {
  ...
  const { Pvcfoamplatesresult, loading, error } = useSelector(
    (state) => state.Pvcfoamplates
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetPvcfoamplatescalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Pvcfoamplatesresult?.message === "Configuration not found") {
      ...
      <div className="bg-[#f1f2f4] flex justify-center items-center w-full">
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">Pvc Foam Plates</h1>
            ...
            <ul className="list-disc list-inside text-sm mb-4">
              <li>Vinyl Print + PVC Foam</li>
              <li>Ecosolvent Print + PVC Foam</li>
              <li>Vinyl Cutting (1 Colour ) + PVC Foam</li>
              <li>Vinyl Cutting (2 Colour ) + PVC Foam</li>
            </ul>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="mb-4">
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="Vinyl Print + PVC Foam">Vinyl Print + PVC Foam</SelectItem>
                    <SelectItem value="Ecosolvent Print + PVC Foam">Ecosolvent Print + PVC Foam</SelectItem>
                    <SelectItem value="Vinyl Cutting (1 Colour ) + PVC Foam">Vinyl Cutting (1 Colour ) + PVC Foam</SelectItem>
                    <SelectItem value="Vinyl Cutting (2 Colour ) + PVC Foam">Vinyl Cutting (2 Colour ) + PVC Foam</SelectItem>
                  </SelectGroup>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={Pvcfoamplatesresult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Pvcfoamplatesresult?.totalPrice || 0}
                    </strong>
                  )}
                </Button>
