...
import { GetNightglowplatescalculation } from "@/lib/ReduxSlice/Rigidsignplate/NightglowplatesSlice";

const Nightglowplates = () => {
  ...
  const { Nightglowplatesresult, loading, error } = useSelector(
    (state) => state.Nightglowplates
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetNightglowplatescalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Nightglowplatesresult?.message === "Configuration not found") {
      setErrorMessage("Selected options are not available");
    } else {
      setErrorMessage("");
    }
  }, [Nightglowplatesresult]);
      ...
      <div className="bg-[#f1f2f4] flex justify-center items-center w-full">
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">Night Glow Plates</h1>
            ...
            <ul className="list-disc list-inside text-sm mb-4">
              <li>Vinyl Cut Letter + Night Glow + PVC Foam</li>
              <li>Vinyl Cut Letter + Night Glow + ACP</li>
            </ul>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="mb-4">
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="Vinyl Cut Letter + Night Glow + PVC Foam">
                      Vinyl Cut Letter + Night Glow + PVC Foam
                    </SelectItem>
                    <SelectItem value="Vinyl Cut Letter + Night Glow + ACP">
                      Vinyl Cut Letter + Night Glow + ACP
                    </SelectItem>
                  </SelectGroup>
                </SelectContent>
              </Select>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={Nightglowplatesresult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Nightglowplatesresult?.totalPrice || 0}
                    </strong>
                  )}
                </Button>
...
export default Nightglowplates;
