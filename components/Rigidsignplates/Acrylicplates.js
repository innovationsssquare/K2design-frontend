...
import { GetAcrylicplatescalculation } from "@/lib/ReduxSlice/Rigidsignplate/AcrylicplatesSlice";
...
const Acrylicplates = () => {
  ...
  const { Acrylicplatesresult, loading, error } = useSelector(
    (state) => state.Acrylicplates
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetAcrylicplatescalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Acrylicplatesresult?.message === "Configuration not found") {
      ...
      <div className="bg-[#f1f2f4] flex justify-center items-center w-full">
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">Acrylic Plates</h1>
            ...
            <ul className="list-disc list-inside text-sm mb-4">
              <li>Acrylic 4mm + Vinyl Letter</li>
              <li>Acrylic 4mm + Golden Vinyl Letter</li>
              <li>Acrylic Letter 2mm + Acrylic back 4mm</li>
              <li>Acrylic Letter 2mm + ACP 3mm</li>
            </ul>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="mb-4">
              <label htmlFor="plateType" className="block mb-2 font-semibold">
                Type
              </label>
              <Select
                onValueChange={(value) =>
                  handleSelectChange("plateType", value)
                }
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="Acrylic 4mm + Vinyl Letter">Acrylic 4mm + Vinyl Letter</SelectItem>
                    <SelectItem value="Acrylic 4mm + Golden Vinyl Letter">Acrylic 4mm + Golden Vinyl Letter</SelectItem>
                    <SelectItem value="Acrylic Letter 2mm + Acrylic back 4mm">Acrylic Letter 2mm + Acrylic back 4mm</SelectItem>
                    <SelectItem value="Acrylic Letter 2mm + ACP 3mm">Acrylic Letter 2mm + ACP 3mm</SelectItem>
                  </SelectGroup>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={Acrylicplatesresult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Acrylicplatesresult?.totalPrice || 0}
                    </strong>
                  )}
                </Button>
