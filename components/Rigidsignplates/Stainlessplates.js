...
import { GetStainlessplatescalculation } from "@/lib/ReduxSlice/Rigidsignplate/StainlessplatesSlice";
...
const Nightglowplates = () => {
  ...
  const { Stainlessplatesresult, loading, error } = useSelector(
    (state) => state.Stainlessplates
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetStainlessplatescalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Stainlessplatesresult?.message === "Configuration not found") {
      setErrorMessage("Selected options are not available");
    } else {
      setErrorMessage("");
    }
  }, [Stainlessplatesresult]);
      ...
      <div className="bg-[#f1f2f4] flex justify-center items-center w-full">
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">Stainless Steel Plates</h1>
            ...
            <ul className="list-disc list-inside text-sm mb-4">
              <li>Stainless Steel Plates + Colour Etching</li>
            </ul>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="mb-4">
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="Stainless Steel Plates + Colour Etching">
                     Stainless Steel Plates + Colour Etching
                    </SelectItem>
                  </SelectGroup>
                </SelectContent>
              </Select>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={Stainlessplatesresult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Stainlessplatesresult?.totalPrice || 0}
                    </strong>
                  )}
                </Button>
