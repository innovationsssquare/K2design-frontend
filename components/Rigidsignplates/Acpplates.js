...
import { GetAcpplatescalculation } from "@/lib/ReduxSlice/Rigidsignplate/acpplatesSlice";
...
const Acpplates = () => {
  ...
  const { Acpplatesresult, loading, error } = useSelector(
    (state) => state.Acpplates
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetAcpplatescalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Acpplatesresult?.message === "Configuration not found") {
      ...
      <div className="bg-[#f1f2f4] flex justify-center items-center w-full">
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">Acp Plates</h1>
            <p className="text-sm text-gray-600 font-bold mb-4">Acp Plates</p>

            <p class="mt-4 text-sm font-medium text-[#606060]">Available In:</p>

            <ul className="list-disc list-inside mb-4">
              <li>Ecosolvent Print + ACP</li>
              <li>Vinyl Cutting (1 Colour) + ACP</li>
              <li>Vinyl Cutting (2 Colour) + ACP</li>
            </ul>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="mb-4">
              ...
              <Select
                onValueChange={(value) => handleSelectChange("plateType", value)}
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="Ecosolvent Print + ACP">
                      Ecosolvent Print + ACP
                    </SelectItem>
                    <SelectItem value="Vinyl Cutting (1 Colour) + ACP">
                      Vinyl Cutting (1 Colour) + ACP
                    </SelectItem>
                    <SelectItem value="Vinyl Cutting (2 Colour) + ACP">
                      Vinyl Cutting (2 Colour) + ACP
                    </SelectItem>
                  </SelectGroup>
                </SelectContent>
              </Select>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={Acpplatesresult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Acpplatesresult?.totalPrice || 0}
                    </strong>
                  )}
                </Button>
