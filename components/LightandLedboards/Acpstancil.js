...
import { GetAcpstancilcalculation } from "@/lib/ReduxSlice/LightandLedboard/acpstancilSlice";
...
const Acpstancil = () => {
  ...
  const { Acpstancilresult, loading, error } = useSelector(
    (state) => state.Acpstancil
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetAcpstancilcalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Acpstancilresult?.message === "Configuration not found") {
      ...
      <div className="bg-[#f1f2f4] flex justify-center items-center w-full">
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">ACP Stencil LED Board</h1>
            <p className="text-sm text-gray-600 font-bold mb-4">
              ACP Stencil LED board with Aluminium profile and LED module.
            </p>

            <p class="mt-4 text-sm font-medium text-[#606060]">Available In:</p>

            <ul className="list-disc list-inside mb-4">
              <li>ACP Stencil LED Board</li>
              <li>Modular ACP Stencil LED Board</li>
            </ul>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="mb-4">
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="ACP Stencil LED Board">ACP Stencil LED Board</SelectItem>
                    <SelectItem value="Modular ACP Stencil LED Board">Modular ACP Stencil LED Board</SelectItem>
                  </SelectGroup>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={Acpstancilresult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Acpstancilresult?.totalPrice || 0}
                    </strong>
                  )}
                </Button>
