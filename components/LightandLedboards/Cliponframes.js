...
import { GetCliponframecalculation } from "@/lib/ReduxSlice/LightandLedboard/cliponframesSlice";
...
const Cliponframes = () => {
  ...
  const { Cliponframeresult, loading, error } = useSelector(
    (state) => state.Cliponframe
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetCliponframecalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Cliponframeresult?.message === "Configuration not found") {
      ...
      <div className="bg-[#f1f2f4] flex justify-center items-center w-full">
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">LED Thinlite Frames (Cliapon Frames)</h1>
            <p className="text-sm text-gray-600 font-bold mb-4">
             LED Thinlite Frames with Translited Print - 75mm Depth Aluminium Profiles
            </p>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="mb-4">
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="Silver Anodised">Silver Anodised</SelectItem>
                    <SelectItem value="Black Anodised">Black Anodised</SelectItem>
                  </SelectGroup>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={Cliponframeresult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Cliponframeresult?.totalCost || 0}
                    </strong>
                  )}
                </Button>
