...
import { GetFabricFramescalculation } from "@/lib/ReduxSlice/LightandLedboard/fabricsledSlice";
...
const Fabricsled = () => {
  ...
  const { Fabricframesresult, loading, error } = useSelector(
    (state) => state.Fabricframes
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetFabricFramescalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Fabricframesresult?.message === "Configuration not found") {
      ...
      <div className="bg-[#f1f2f4] flex justify-center items-center w-full">
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">Fabrics LED board / Textile LED board</h1>
            <p className="text-sm text-gray-600 font-bold mb-4">
             LED Fabrics Frames 75mm Depth + Aluminium profile (Silver Anodised) + LED Module
            </p>

            <p class="mt-4 text-sm font-medium text-[#606060]">Available In:</p>

            <ul className="list-disc list-inside mb-4">
              <li>LED Fabrics Frames 75mm Depth</li>
              <li>Aluminium profile (Silver Anodised)</li>
              <li>LED Module</li>
            </ul>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="mb-4">
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="LED Fabrics Frames 75mm Depth">LED Fabrics Frames 75mm Depth</SelectItem>
                    <SelectItem value="Aluminium profile (Silver Ano.)">Aluminium profile (Silver Anodised)</SelectItem>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={Fabricframesresult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Fabricframesresult?.totalPrice || 0}
                    </strong>
                  )}
                </Button>
