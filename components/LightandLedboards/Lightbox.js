...
import { GetLightboxBoardcalculation } from "@/lib/ReduxSlice/LightandLedboard/LightboxSlice";
...
const Lightbox = () => {
  ...
  const { LightboxBoardresult, loading, error } = useSelector(
    (state) => state.LightboxBoard
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetLightboxBoardcalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (LightboxBoardresult?.message === "Configuration not found") {
      ...
      <div className="bg-[#f1f2f4] flex justify-center items-center w-full">
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">Lightbox Board</h1>
            <p className="text-sm text-gray-600 font-bold mb-4">
              Lightbox board options with Backlit Flex or Acrylic and LED
              modules.
            </p>
            ...
            <ul className="list-disc list-inside mb-4">
              <li>{`Backlit Flex + LED Module 3'' Depth`}</li>
              <li>{`Acrylic + Vinyl cut letter + LED Module 3''depth board`}</li>
              <li>{`Acrylic + Acrylic cut letter + LED Module 3''depth board`}</li>
              <li>{`Backlit flex + LED Strip 3''depth board`}</li>
            </ul>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="mb-4">
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="Backlit Flex + LED Module 3'' Depth">{`Backlit Flex + LED Module 3'' Depth`}</SelectItem>
                    <SelectItem value="Acrylic + Vinyl cut letter + LED Module 3''depth board">{`Acrylic + Vinyl cut letter + LED Module 3''depth board`}</SelectItem>
                    <SelectItem value="Acrylic + Acrylic cut letter + LED Module 3''depth board">{`Acrylic + Acrylic cut letter + LED Module 3''depth board`}</SelectItem>
                    <SelectItem value="Backlit flex + LED Strip 3''depth board">{`Backlit flex + LED Strip 3''depth board`}</SelectItem>
                  </SelectGroup>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={LightboxBoardresult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{LightboxBoardresult?.totalPrice || 0}
                    </strong>
                  )}
                </Button>
