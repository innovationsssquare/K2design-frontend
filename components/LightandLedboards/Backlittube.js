...
import { GetBacklittubecalculation } from "@/lib/ReduxSlice/LightandLedboard/backlittubeSlice";
...
const Backlittube = () => {
  ...
  const { Backlittuberesult, loading, error } = useSelector(
    (state) => state.Backlittube
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetBacklittubecalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Backlittuberesult?.message === "Configuration not found") {
        ...
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">
              Backlit (Tube) Flex Board
            </h1>
            <p className="text-sm text-gray-600 font-bold mb-4">
              Backlit tube flex board with different size-based pricing.
            </p>

            <p class="mt-4 text-sm font-medium text-[#606060]">Available In:</p>

            <ul className="list-disc list-inside mb-4">
              <li>Backlit (Tube) Flex Board 1 side</li>
              <li>Backlit (Tube) Flex Board 2 side</li>
              <li>Lengthy Backlit (Tube) Flex Board 1 side</li>
            </ul>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="mb-4">
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="Backlit (Tube) Flex Board 1 side">Backlit (Tube) Flex Board 1 side</SelectItem>
                    <SelectItem value="Backlit (Tube) Flex Board 2 side">Backlit (Tube) Flex Board 2 side</SelectItem>
                    <SelectItem value="Lengthy Backlit (Tube) Flex Board 1 side">Lengthy Backlit (Tube) Flex Board 1 side</SelectItem>
                  </SelectGroup>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={Backlittuberesult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Backlittuberesult?.totalPrice || 0}
                    </strong>
                  )}
                </Button>
