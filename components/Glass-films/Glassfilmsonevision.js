...
import { GetGlassfilmsonevisioncalculation } from "@/lib/ReduxSlice/Glass-films/glassfimonevisionSlice";

const Glassfilmsonevision = () => {
  ...
  const { Glassfilmonevisionprintresult, loading, error } = useSelector(
    (state) => state.Glassfilmonevisionprint
  );
  ...
  useEffect(() => {
    if (formData.width && formData.height) {
      dispatch(GetGlassfilmsonevisioncalculation(formData));
    }
  }, [formData, dispatch]);

  useEffect(() => {
    if (Glassfilmonevisionprintresult?.message === "Configuration not found") {
      setErrorMessage("Selected options are not available");
    } else {
      setErrorMessage("");
    }
  }, [Glassfilmonevisionprintresult]);
        ...
        <div className="w-full p-4 shadow-sm ml-4 mr-4 my-4 flex gap-4 justify-center    lg:flex-row flex-col bg-white">
          ...
          <div className=" px-5 w-full">
            <h1 className="text-2xl font-bold mb-4">
              One Way Vision Print (Glass Film)
            </h1>
            <p className="text-sm text-gray-600 font-bold mb-4">
              High-quality One way vision print on glass film for clear
              visibility.
            </p>
            ...
            <ul className="list-disc list-inside mb-4">
              <li>glass film Economy</li>
              <li>glass film Premium</li>
            </ul>
          ...
          {`We do not accept designs that belong to or represent government or government-affiliated organizations.`}
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <Button
                  className="bg-white  "
                  disabled={Glassfilmonevisionprintresult == null}
                >
                  {loading ? (
                    <span className="loader4"></span>
                  ) : (
                    <strong className="text-Apptheme text-lg ">
                      ₹{Glassfilmonevisionprintresult?.totalPrice || 0}
                    </strong>
                  )}
                </Button>
...
export default Glassfilmsonevision;
